...
def write_with_comment(file, code, value, comment):
   file.write(code.format(value))
   add_spaces(file, 10, value)
   file.write(comment)
   file.write('\n')
...
def print_convolution_call(source_file, layer, weight_ptr, input_ptr):
   ...
   if layer.use_bias:
      bias_ptr = weight_ptr + weight_size
   else:
      bias_ptr = 0

   source_file.write(' \n')
   source_file.write('   conv2d_sw( \n')
   write_with_comment(source_file, '       memory + {:d}, ', input_ptr,             '// offset of input images ')
   write_with_comment(source_file, '       memory + {:d}, ', weight_ptr,            '// offset of weights ');
   write_with_comment(source_file, '       memory + {:d}, ', bias_ptr,              '// offset of biases ')
   write_with_comment(source_file, '       memory + {:d}, ', input_ptr+in_size,     '// offset of output images ')
   write_with_comment(source_file, '       {:d},          ', layer.input_shape[3],  '// number of input images ')
   write_with_comment(source_file, '       {:d},          ', layer.output_shape[3], '// number of output images ')
   write_with_comment(source_file, '       {:d},          ', layer.input_shape[1],  '// height ')
   write_with_comment(source_file, '       {:d},          ', layer.input_shape[2],  '// width ')
...
def print_dense_call(source_file, layer, weight_ptr, input_ptr):
   ...
   write_with_comment(source_file, '       memory + {:d}, ', bias_ptr,              '// offset of biases ')
   write_with_comment(source_file, '       memory + {:d}, ', input_ptr + in_size,   '// offset of output images ')
   write_with_comment(source_file, '       {:d},          ', 1,                     '// number of rows in input image ')
   write_with_comment(source_file, '       {:d},          ', layer.kernel.shape[0], '// number of cols in input image ')
   write_with_comment(source_file, '       {:d},          ', layer.kernel.shape[1], '// number of output images ')
...
#===== Print hw inference function calls =====
...
def hw_print_convolution_call(source_file, layer, weight_ptr, input_ptr):
   ...
   write_with_comment(source_file,'       {:d},          ', input_ptr,             '// offset of input images  ')
   write_with_comment(source_file,'       {:d},          ', weight_ptr,            '// offset of weights       ')
   write_with_comment(source_file,'       {:d},          ', bias_ptr,              '// offset of biases        ')
   write_with_comment(source_file,'       {:d},          ', input_ptr + in_size,   '// offset of output images ')
   write_with_comment(source_file,'       {:d},          ', layer.input_shape[3],  '// number of input images  ')
   write_with_comment(source_file,'       {:d},          ', layer.output_shape[3], '// number of output images ')
   write_with_comment(source_file,'       {:d},          ', layer.input_shape[1],  '// height                  ')
   write_with_comment(source_file,'       {:d},          ', layer.input_shape[2],  '// width                   ')
...
def hw_print_dense_call(source_file, layer, weight_ptr, input_ptr):
   ...
   write_with_comment(source_file, '       {:d},          ', bias_ptr,                '// offset of biases               ')
   write_with_comment(source_file, '       {:d},          ', input_ptr + in_size,     '// offset of output images        ')
   write_with_comment(source_file, '       {:d},          ', 1,                       '// number of rows in input images ')
   write_with_comment(source_file, '       {:d},          ', layer.kernel.shape[0],   '// number of cols in input images ')
   write_with_comment(source_file, '       {:d},          ', layer.kernel.shape[1],   '// number of output images        ')
...
def write_convolution_weights(layer, n, header, data, source, weight_ptr, input_ptr, max_pool, max_pool_shape):
   ...
   for out_image in range(layer.kernel.shape[3]):
      for in_image in range(layer.kernel.shape[2]): 
         for r in range(layer.kernel.shape[0]):
            for c in range(layer.kernel.shape[1]):
